...
import 'details_test/video_details_screen_details_test.dart';

class PlantDetectionsVideoScreen extends StatelessWidget {
  VideoPlayerController video;

  PlantDetectionsVideoScreen({super.key, required this.video});
      ...
      child: BlocConsumer<DetectionCubit, DetectionStates>(
        listener: (context, state) {
          if (state is DetectionErrorState) {
            ...
            navigateAndFinish(context, PlantLayoutScreen());
          ...
          } else if (state is DetectionVideoSuccessState) {
            showCustom(context, state.message, Colors.green, Icons.check);
            navigateAndFinish(
                context,
                VideoDetailsScreenDetailTest(
                  des: state.modelViedoDetails,
                  video: VideoPlayerController.file(
                      state.modelViedoDetails.video!),
                ));
              ...
              child: ListView(
                ...
                children: <Widget>[
                  ...
                  ListView.builder(
                    ...
                    itemCount: cubit.videoAvailableModels.length,
                    itemBuilder: (context, index) {
                      String model = cubit.videoAvailableModels[index];
                      String description =
                          cubit.videoFeatureDescriptions[index];
